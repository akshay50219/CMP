          ...
          datasets: [
            {
              ...
              data: data.submissionTrends?.data || [],
    ...
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button
            variant="outlined"
            startIcon={<Download />}
            onClick={generateConferenceProgram}
          >
            Generate Program
          </Button>
          <Button
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        {[
          ...
          { 
            ...
            color: 'success',
            path: '/admin/papers?status=accepted'
          },
          { 
            ...
            color: 'error',
            path: '/admin/papers?status=rejected'
          },
        ].map((stat, index) => (
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid item xs={12} md={8}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ height: 300 }}>
              {chartData.submissionTrends?.labels?.length > 0 ? (
                <Bar
                  data={chartData.submissionTrends}
                  options={{
                    responsive: true,
              ...
              <Table size="small">
                ...
                <TableBody>
                  {recentPapers.map((paper) => (
                    <TableRow key={paper._id} hover sx={{ cursor: 'pointer' }} onClick={() => navigate(`/admin/papers/${paper._id}`)}>
                      <TableCell>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {recentUsers.map((user) => (
                    <TableRow key={user._id} hover sx={{ cursor: 'pointer' }} onClick={() => navigate(`/admin/users/${user._id}`)}>
                      <TableCell>
                        <Typography variant="body2">
                          {user.name}
                        </Typography>
                      ...
                      <TableCell>
                        <Chip
                          label={user.role}
                          color={user.role === 'admin' ? 'error' : user.role === 'reviewer' ? 'warning' : 'info'}
                          size="small"
                        />
                      </TableCell>
                      <TableCell>
      ...
      <Paper sx={{ p: 3, mt: 3, bgcolor: 'primary.light' }}>
        ...
        <Grid container spacing={2}>
          {[
            { label: 'Assign Reviewers', action: () => navigate('/admin/assign'), icon: <Assignment /> },
            { label: 'View Statistics', action: () => navigate('/admin/stats'), icon: <BarChart /> },
            { label: 'Manage Users', action: () => navigate('/admin/users'), icon: <People /> },
            { label: 'Review Decisions', action: () => navigate('/admin/papers?status=under_review'), icon: <Assessment /> },
          ].map((action, index) => (
